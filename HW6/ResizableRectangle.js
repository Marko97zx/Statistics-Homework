...
class ResizableRectangle {
    ...
    drawHistogramChart(c, histo, drawAtY) {
        let maxY = c.maxValue, minY = c.minValue;
        let maxValue = histo.effectiveMaxValue, minValue = histo.effectiveMinValue;
    ...
    drawChartNormalOG(c) {
        ...
        const pointsList = c.coordinates;
        const trajectoriesColor = c.trajectoriesColor;

        this.drawRectangle();

        for (let k = 0; k < pointsList.length; k++) {
            
            const scaledPoints = new Array(pointsList[k].length);

            for (let i = 0; i < scaledPoints.length; i++) {
                scaledPoints[i] = new Point(
                    ResizableRectangle.linearTransformX(this.rectangle.left + 1, pointsList[k][i].x, 0, c.n - 1, this.rectangle.width - 2),
                    ResizableRectangle.linearTransformY(this.rectangle.top + 1, pointsList[k][i].y, c.minValue, c.maxValue, this.rectangle.height - 2)
                );
            }
        ...
        this.drawHistogramChart(c, c.endValuesHistogram, this.rectangle.right + 2);
        this.drawHistogramChart(c, c.midValuesHistogram, ResizableRectangle.linearTransformX(this.rectangle.left + 1, c.midValuesHistogram.calculationPoint, 0, c.n - 1, this.rectangle.width - 2));
    }
    ...
    drawChartNormal(c) {
        ...
        for (let k = 0; k < pointsList.length; k++) {
            if (this.#whiteToggle && trajectoriesColor[k] == "White") continue;
            ...
            this.ctx.strokeStyle = trajectoriesColor[k] == "Black" ? this.outlineColor : trajectoriesColor[k];
            this.ctx.beginPath();
            this.ctx.moveTo(scaledPoints[0].x, scaledPoints[0].y);
            for (let i = 1; i < scaledPoints.length; i++) {
                this.ctx.lineTo(scaledPoints[i].x, scaledPoints[i].y);
        ...
        this.drawProbabilitiesChart(c);
    }


    drawProbabilitiesChart(c) {

        const maxPointY = ResizableRectangle.linearTransformY(this.rectangle.top + 1, c.k * 10, c.minValue, c.maxValue, this.rectangle.height - 2);
        const minPointY = ResizableRectangle.linearTransformY(this.rectangle.top + 1, 20, c.minValue, c.maxValue, this.rectangle.height - 2);
    
        const histoBaseLength = maxPointY - minPointY;
        const barBaseSize = histoBaseLength / (c.k - 2);

        for(let k = 0; k < c.probabilities.length; k++)
        {
            const point = new Point(this.rectangle.right + 2, minPointY + k*barBaseSize);
           
            this.ctx.fillStyle = this.outlineColor;
            this.ctx.fillRect(point.x, point.y, (this.rectangle.width / 4) * c.probabilities[k], barBaseSize);

            this.ctx.strokeStyle = ResizableRectangle.getAnalogousColors(this.outlineColor);
            this.ctx.strokeRect(point.x, point.y, (this.rectangle.width / 4) * c.probabilities[k], barBaseSize);
